...
PORT_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    ...
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FTAAMQ7S-if00-port0": DynamixelRobotConfig(
        joint_ids=(1, 2, 3, 4, 5, 6, 7),
        joint_offsets=(
            2 * np.pi/2,
            2 * np.pi/2,
        ...
        joint_signs=(1, 1, 1, 1, 1, -1, 1), # ORIGINAL WITH joint 3 and Joint 5 bugs
        #joint_signs=(1, -1, -1, 1, -1, -1, 1),
        gripper_config=(8, 203, 161),
    ),
    

    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT3R484D-if00-port0": DynamixelRobotConfig(
        joint_ids=(1, 2, 3, 4, 5, 6, 7),
        joint_offsets=(
            3 * np.pi / 2,
            2 * np.pi / 2,
